...
(slide
 ...
 (Code (define (instantiate proto base)
         (define self (proto (λ (x) (self x)) base))
         self))
 (Code
...
(slide
 ...
 @Code[
   (define (myPoint m) (case m ((x) 1) ((y) 2) (else (⊥))))
 ]
 'next
 @Code[
   (define ($x3 self super)
     (λ (msg) (if (eq? msg 'x) 3 (super msg))))
   (define ($doubleX self super)
 ...
 'next
 @Code[
   (define $yourPoint (inherit $z<-xy (inherit $doubleX $x3)))
   (define yourPoint (instantiate $yourPoint myPoint))
   > (yourPoint 'z)
   6+2i]
 @comment{
...
(slide
 ...
 (Code
  ...
  (define (instantiate-generator gen)
    (define self (gen (λ (x) (self x)))) self)
  \ 
  (code:comment "proto->generator : (Fun (Proto A B) B -> (Gen A))")
  (define (proto->generator p b) (λ (self) (p self b)))
...
(slide
 #:title "Dependencies, Linearization and Modularity"
 @P{@tt{A B C D E}  each directly depends on @tt{O}}
 @P{@tt{K}  directly depends on @tt{A B C}}
 @P{@tt{L}  directly depends on @tt{B D E}}
 @P{@tt{M}  directly depends on @tt{D A}}
 @P{@tt{Z}  directly depends on @tt{K L M}}
 (blank-line)
 @P{For @tt{Z}  mix @tt{Z K L M D A B C E O}})
...
#|
I was raised in academia where Functional Programming, FP, reigns supreme.
And I made career in the industry, where Object Orientation, OO, is The Thing.
For 30 years, I've wanted an explanation in the terms of FP of what OO is and why it matters.
But OO and FP people have always talked past each other. Yet one day I discovered... Nix.
And that's how I found the explanation, that I will now share with you.
...
Elucidating its fundamental concepts.
If you understand those concepts, everything about OO will be clear to you,
you'll know what to expect and demand from it,
how to use it and what for — or not.
If you don't, OO will remain but confusing sound and fury,
that you 


So, what is the essence of OO?
