...
;; These tests only work in Gerbil Scheme, but you may easily port them to
;; your Scheme implementation of choice, or non-Scheme language of choice.
...
(import
  ...
  (only-in :std/test test-suite test-case check check-exception run-tests!)
...
(defhierarchy (my-objects my-supers my-descriptors)
  ...
  ;; Examples that mix structs and traits
...
(def (my-compute-precedence-list x)
  (hash-ensure-ref
   ...
   (cut first-value
        (c4-linearization [x] (my-get-supers x) ;; rely on values being reified first-class objects
